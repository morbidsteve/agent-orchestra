...
if [ "$_in_container" = false ] && [ "${ORCHESTRA_ALLOW_HOST:-}" != "true" ]; then
    echo ""
    echo -e "${YELLOW}╔══════════════════════════════════════════════════════════════╗${NC}"
    echo -e "${YELLOW}║  NOTE: Running outside a container                          ║${NC}"
    echo -e "${YELLOW}╠══════════════════════════════════════════════════════════════╣${NC}"
    echo -e "${YELLOW}║  The dashboard and API will work normally, but agent         ║${NC}"
    echo -e "${YELLOW}║  execution is disabled — agents use unrestricted filesystem  ║${NC}"
    echo -e "${YELLOW}║  access and need container isolation.                        ║${NC}"
    echo -e "${YELLOW}║                                                              ║${NC}"
    echo -e "${YELLOW}║  To enable agents: use a devcontainer or Docker.             ║${NC}"
    echo -e "${YELLOW}║  To enable agents here: export ORCHESTRA_ALLOW_HOST=true     ║${NC}"
    echo -e "${YELLOW}╚══════════════════════════════════════════════════════════════╝${NC}"
    echo ""
fi
